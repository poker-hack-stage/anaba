...
穴場度（`hidden_gem_score`）の列は #61 で足すので、この PR の seed には入れていない。#61 のマージ後に `supabase/pending/hidden_gem_scores.sql` の値を seed の insert に移し、pending を消してからこの PR をマージする。
